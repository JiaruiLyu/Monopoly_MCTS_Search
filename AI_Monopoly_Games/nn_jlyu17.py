...
def predict(model: Net, game: Game) ->int:
    ...
    game_data = numpy.append(game_data, game.get_score(0)) # append the score
    game_data = numpy.append(game_data, game.get_score(1)) # append the score
...
def run_experiments(nn_model):
    ...
    print("Jlyu NN-AI Experiment is ready to start!, grid size = {}, player zero is a Baseline AI, player one is a MCTS AI".format(grid_size))
...
if __name__ == "__main__":
    ...
    nn_model = configure_data(train_data, test_data)
